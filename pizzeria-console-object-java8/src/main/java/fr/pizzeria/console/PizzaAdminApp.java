...
public class PizzaAdminApp {
	public static void main(String[] args) throws IOException {
		...
		try (ClassPathXmlApplicationContext context = new ClassPathXmlApplicationContext(option,
			...
			Menu menu = context.getBean("menu",
					Menu.class);/*
								 * Menu menu = new Menu(new
								 * QuitterOptionMenu(sc, pizzaDao),
								 * "admin console", sc, new
								 * ListerPizzaOptionMenu(sc, pizzaDao), new
								 * AjouterPizzaOptionMenu(sc, pizzaDao), new
								 * ModifierPizzaMenuOption(sc, pizzaDao), new
								 * SupprimerPizzaOptionMenu(sc, pizzaDao), new
								 * ListerPizzaByCategorieOptionMenu(sc,
								 * pizzaDao), new ListerPizzaPrixOptionMenu(sc,
								 * pizzaDao));
								 */
			IPizzaDao pizzaDao = (IPizzaDao) context.getBean("dao");//on pourais faire un menu.getDao()
			...
			menu.setActions(new QuitterOptionMenu(sc, pizzaDao), new ListerPizzaOptionMenu(sc, pizzaDao),
					...
					new ListerPizzaPrixOptionMenu(sc, pizzaDao));
			menu.afficher();
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("");
